...
exports.searchRegionSetup = searchRegionSetup;
exports.findQuake = findQuake;

function extractStartAndEnd(dateTimeExpression, args) {
  var whenStart;
  var whenEnd;
  ...
  args.starttime = whenStart.toIsoString();
  args.endtime = whenEnd.toIsoString();
}

function searchRegionSetup(searchRegion, args) {
...
function findQuake(url, args, ret) {
  var str = http.getUrl(url, {
    query: args
  })
  ...
  for (var i = 0; i < res.features.length; i++) {
    ...
    var geo = {
      latitude: res.features[i].geometry.coordinates[1],
      longitude: res.features[i].geometry.coordinates[0]
    }
    var zonedDT = new dates.ZonedDateTime(geo, res.features[i].properties.time)
    var event_type = res.features[i].properties.type;
